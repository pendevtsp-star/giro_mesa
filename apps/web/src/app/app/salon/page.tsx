"use client";
import { LayoutGrid, Plus, Save, Users } from "lucide-react";
...
export default function SalonPage() {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: bootstrap da filial e do mapa ao abrir a tela.
  useEffect(() => {
    void (async () => {
      try {
  ...
  function drop(event: DragEvent<HTMLElement>) {
    ...
    setLayout((current) => ({
      ...current,
      [dragged]: {
        x: Math.max(0, Math.min(88, ((event.clientX - rect.left) / rect.width) * 100)),
        y: Math.max(0, Math.min(82, ((event.clientY - rect.top) / rect.height) * 100)),
      },
    }));
    setDragged(null);
  }
  async function persist() {
    if (!branchId) return;
    await saveFloorPlan(branchId, layout);
    setMessage("Disposição do salão salva para esta unidade.");
  }
  async function submit(e: FormEvent) {
    ...
    await load();
    setMessage("Mesa criada. Arraste-a para posicionar no mapa.");
  }
  return (
    <main className="salon-page">
      <header className="kds-topbar">
        ...
        <div className="toolbar">
          <button className="button primary compact" onClick={() => void persist()} type="button">
            <Save size={16} /> Salvar mapa
          </button>
          <a className="button secondary compact" href="/app?view=pos">
            Abrir PDV
          </a>
        </div>
      </header>
      ...
      <section
        aria-label="Mapa posicionavel de mesas"
      ...
      >
        {tables.map((table, index) => {
          ...
          return (
            <a
              draggable
              className={`salon-table salon-positioned ${tones[table.status] ?? "reserved"}`}
